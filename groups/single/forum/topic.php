<?php
/**
 * BuddyPress - Group New Forum Topic
 * 
 * @package BuddyPress
...
<form action="<?php bp_forum_topic_action(); ?>" method="post" id="forum-topic-form" class="standard-form">
	...
	<div class="item-list-tabs no-ajax" id="subnav" role="navigation">
		<ul>
			...
			<?php if ( is_user_logged_in() ) : ?>
				...
				<li>
					<a href="<?php bp_forum_topic_new_reply_link(); ?>" class="new-reply-link"><?php _e( 'New Reply', 'bp-default' ); ?></a>
				</li>
	...
	<div id="topic-meta">
		...
		<?php if ( bp_forum_topic_has_tags() ) : ?>
			...
			<div class="topic-tags">
				<?php _e( 'Topic tags:', 'bp-default' ); ?> <?php bp_forum_topic_tag_list(); ?>
			</div><!-- .topic-tags -->
			...
			<?php while ( bp_forum_topic_posts() ) : bp_the_forum_topic_post(); ?>
				...
				<li id="post-<?php bp_the_topic_post_id(); ?>" class="<?php bp_the_topic_post_css_class(); ?>">
					...
					<div class="poster-meta">
						...
						<?php echo sprintf( __( '%1$s said %2$s:', 'bp-default' ), bp_get_the_topic_post_poster_name(), bp_get_the_topic_post_time_since() ); ?>
					...
					<div class="admin-links">
						...
						<a href="#post-<?php bp_the_topic_post_id(); ?>" title="<?php esc_attr_e( 'Permanent link to this post', 'bp-default' ); ?>">#</a>

					</div><!-- .admin-links -->

				</li><!-- #post-bp_the_topic_post_id() -->
	...
	<?php else: ?>
		...
		<div id="message" class="info">
			<p><?php _e( 'There are no posts for this topic.', 'bp-default' ); ?></p>
		</div><!-- #message -->
	...
	<?php if ( ( is_user_logged_in() && 'public' == bp_get_group_status() ) || bp_group_is_member() ) : ?>
		...
		<?php if ( bp_get_the_topic_is_last_page() ) : ?>
			...
			<?php if ( bp_get_the_topic_is_topic_open() && !bp_group_is_user_banned() ) : ?>
				...
				<div id="post-topic-reply">
					...
					<?php if ( bp_groups_auto_join() && !bp_group_is_member() ) : ?>
						<p><?php _e( 'You will auto join this group when you reply to this topic.', 'bp-default' ); ?></p>
					<?php endif; ?>

					<?php do_action( 'groups_forum_new_reply_before' ); ?>

					<h4><?php _e( 'Add a reply:', 'bp-default' ); ?></h4>

					<textarea name="reply_text" id="reply_text"></textarea>

					<div class="submit">
						<input type="submit" name="submit_reply" id="submit" value="<?php esc_attr_e( 'Post Reply', 'bp-default' ); ?>" />
					</div><!-- .submit -->
			...
			<?php elseif ( !bp_group_is_user_banned() ) : ?>
				...
				<div id="message" class="info">
					<p><?php _e( 'This topic is closed, replies are no longer accepted.', 'bp-default' ); ?></p>
				</div><!-- #message -->
